...
<?php
  ...
  if(isset($_GET['u_id'])){
    ...
    $Sname=$row2['name'];
    $sql3 = "SELECT * FROM dashboard WHERE isbn ='$isbn';";
    $arr3=mysqli_query($conection,$sql3);
    $row3=mysqli_fetch_array($arr3);
    $Bname=$row3['book_name'];
    $sql4 = "SELECT * FROM dashboard WHERE isbn ='$isbn' and u_id ='$username';;";
    $arr4=mysqli_query($conection,$sql4);
    $row4=mysqli_fetch_array($arr4);
    $Returned_date=$row3['reg_date'];
